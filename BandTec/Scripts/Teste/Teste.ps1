Param ( 
$computers = (Get-Content  "C:\Users\mikazevedo\Desktop\Scripts\Teste\ListaServidores.txt") 
) 
...
ForEach ($computer in $groups) { 
    ...
    @{Name="Tipo de disco";Expression={-f (Get-PhysicalDisk | select MediaType)}},
     @{
         name="MediaType"; 
         expression={
             switch ($_.MediaType) { 
...
ConvertTo-Html -head $head -body $fragments  | Out-File C:\Users\mikazevedo\Desktop\Scripts\Teste\Admin_EspacoemDisco.html